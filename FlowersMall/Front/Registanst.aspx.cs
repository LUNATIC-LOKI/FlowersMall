...
public partial class Registanst : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (!db.Fault)//判断是否成功连接数据库
        {
            ...
            if (!db.QueryValue(TextBox1.Text, 1))//查询本地数据库，判断用户名是否存在
            {
                DataRow dr = db.MyDataSet.Tables[0].NewRow();//根据本地数据库中，表的结构，复制一条空行
                dr.BeginEdit();//开始编辑行

                dr["u_id"] = radomNum();
                dr["u_name"] = TextBox1.Text;
                dr["u_iphone"] = TextBox2.Text;
                dr["u_password"] = TextBox4.Text;
                dr["u_mail"] = TextBox5.Text;
                ...
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "message", "<script language='javascript' defer>alert('注册成功！');window.location='../Front/Login.aspx';</script>");
            }
            else
            {
                db.OffData();
